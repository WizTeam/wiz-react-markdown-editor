...
class Selection {
  ...
  findMatchingSelectionParent(testElementFunction, contentWindow) {
    ...
    return (0, _dom.traverseUp)(current, testElementFunction);
  ...
  importSelection(selectionState, root, favorLaterSelectionAnchor) {
    ...
    let lastTextNode = null; // When importing selection, the start of the selection may lie at the end of an element
  ...
  importSelectionMoveCursorPastBlocks(root, index = 1, range) {
    ...
    let startBlock;
    let targetNode;
    let currIndex = 0; // If index is 0, we still want to move to the next block
  ...
  getCursorRange() {
    ...
    if (!isAnchorValid && isFocusValid) {
      ...
      const editor = document.querySelector("#".concat(this.muya.CLASS_OR_ID.AG_EDITOR_ID)).parentNode;
      editor.blur();
      return new _cursor.default({
        start: null,
        end: null,
        anchor: null,
        focus: null
    ...
    const anchorParagraph = (0, _dom.findNearestParagraph)(anchorNode);
    const focusParagraph = (0, _dom.findNearestParagraph)(focusNode);
    ...
    const result = new _cursor.default({
      anchor,
      focus
    });
