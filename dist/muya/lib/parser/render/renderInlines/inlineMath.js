...
function displayMath(h, cursor, block, token, outerClass) {
  ...
  const mathSelector =
    className === _config.CLASS_OR_ID.AG_HIDE
      ? 'span.'.concat(className, '.').concat(_config.CLASS_OR_ID.AG_MATH)
      : 'span.'.concat(_config.CLASS_OR_ID.AG_MATH);
  ...
  return [
    h('span.'.concat(className, '.').concat(_config.CLASS_OR_ID.AG_MATH_MARKER), startMarker),
    h(mathSelector, [
      h(
        'span.'
          .concat(_config.CLASS_OR_ID.AG_INLINE_RULE, '.')
          .concat(_config.CLASS_OR_ID.AG_MATH_TEXT),
        {
          attrs: {
            spellcheck: 'false'
          }
    ...
    h('span.'.concat(className, '.').concat(_config.CLASS_OR_ID.AG_MATH_MARKER), endMarker)
  ];
}
