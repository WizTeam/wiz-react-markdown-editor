...
function emoji(h, cursor, block, token, outerClass) {
  ...
  const contentSelector =
    finalClass !== _config.CLASS_OR_ID.AG_GRAY
      ? 'span.'
          .concat(finalClass, '.')
          .concat(_config.CLASS_OR_ID.AG_INLINE_RULE, '.')
          .concat(_config.CLASS_OR_ID.AG_EMOJI_MARKED_TEXT)
      : 'span.'
          .concat(_config.CLASS_OR_ID.AG_INLINE_RULE, '.')
          .concat(_config.CLASS_OR_ID.AG_EMOJI_MARKED_TEXT);
  let startMarkerSelector = 'span.'
    .concat(finalClass, '.')
    .concat(_config.CLASS_OR_ID.AG_EMOJI_MARKER);
