...
function link(h, cursor, block, token, outerClass) {
  ...
  const linkClassName =
    className === _config.CLASS_OR_ID.AG_HIDE ? className : _config.CLASS_OR_ID.AG_LINK_IN_BRACKET;
  ...
  if (
    (0, _utils.isLengthEven)(token.backlash.first) &&
    (0, _utils.isLengthEven)(token.backlash.second)
  ) {
    if (!token.children.length && !token.backlash.first) {
      ...
      return [
        ...
        h(
          'a.'
            .concat(_config.CLASS_OR_ID.AG_NOTEXT_LINK, '.')
      ...
      // has children
      return [
        h('span.'.concat(className, '.').concat(_config.CLASS_OR_ID.AG_REMOVE), firstBracket),
        h(
          'a.'.concat(_config.CLASS_OR_ID.AG_INLINE_RULE),
          {
            props: {
              href: token.href + encodeURI(token.backlash.second),
              target: '_blank',
          ...
          [
            ...token.children.reduce((acc, to) => {
              const chunk = this[(0, _utils.snakeToCamel)(to.type)](
                h,
                cursor,
                block,
                to,
        ...
        h('span.'.concat(className, '.').concat(_config.CLASS_OR_ID.AG_REMOVE), middleBracket),
        h(
          'span.'.concat(linkClassName, '.').concat(_config.CLASS_OR_ID.AG_REMOVE),
          {
            attrs: {
              spellcheck: 'false'
            }
        ...
        h('span.'.concat(className, '.').concat(_config.CLASS_OR_ID.AG_REMOVE), lastBracket)
      ];
    }
  } else {
    return [
