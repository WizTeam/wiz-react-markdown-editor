...
const getHighlightHtml = (text, highlights, escape = false, handleLineEnding = false) => {
  ...
  const getEscapeHTML = (className, content) => {
    return ''
      .concat(MARKER_HASK['<'], 'span class=')
      .concat(MARKER_HASK['"'])
      .concat(className)
      .concat(MARKER_HASK['"'])
      .concat(MARKER_HASK['>'])
      .concat(content)
      .concat(MARKER_HASK['<'], '/span')
      .concat(MARKER_HASK['>']);
  };
  ...
  for (const highlight of highlights) {
    ...
    code += escape
      ...
      : '<span class="'.concat(className, '">').concat(highlightContent, '</span>');
    pos = end;
  }
...
function renderLeafBlock(parent, block, activeBlocks, matches, useCache = false) {
  ...
  if (text) {
    ...
    if (highlights.length === 0 && this.tokenCache.has(text)) {
      ...
      _config.HAS_TEXT_BLOCK_REG.test(type) &&
    ...
    children = tokens.reduce(
      (acc, token) => [
        ...acc,
        ...this[(0, _utils.snakeToCamel)(token.type)](_snabbdom.h, cursor, block, token)
      ],
      []
    );
  }
  ...
  if (type === 'div') {
    ...
    switch (functionType) {
      case 'html': {
        ...
        if (/^<([a-z][a-z\d]*)[^>]*?>(\s*)<\/\1>$/.test(htmlContent.trim())) {
          children = (0, _snabbdom.htmlToVNode)(
    ...
    selector = ''
      .concat(type, '#')
      .concat(key, '.')
      .concat(_config.CLASS_OR_ID.AG_TASK_LIST_ITEM_CHECKBOX);
    ...
    const code = (0, _utils.escapeHtml)(getHighlightHtml(text, highlights, true, true))
      .replace(new RegExp(MARKER_HASK['<'], 'g'), '<')
      .replace(new RegExp(MARKER_HASK['>'], 'g'), '>')
      .replace(new RegExp(MARKER_HASK['"'], 'g'), '"')
      .replace(new RegExp(MARKER_HASK["'"], 'g'), "'"); // transfrom alias to original language
