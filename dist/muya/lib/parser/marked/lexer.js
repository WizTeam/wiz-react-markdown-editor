...
function Lexer(opts) {
  ...
  this.options = Object.assign({}, _options.default, opts);
  this.rules = _blockRules.normal;

  if (this.options.pedantic) {
    this.rules = _blockRules.pedantic;
  } else if (this.options.gfm) {
    this.rules = _blockRules.gfm;
  }
}
/**
 * Preprocessing
...
Lexer.prototype.token = function (src, top) {
  ...
  let i;
  let tag;
  let l; // Only check front matter at the begining of a markdown file.
  ...
  while (src) {
    ...
    if (cap) {
      ...
      if (lastToken && lastToken.type === 'paragraph') {
        ...
        this.tokens.push({
          ...
          text: !this.options.pedantic ? (0, _utils.rtrim)(cap, '\n') : cap
        });
      }
    ...
    if (footnote) {
      ...
      if (top && cap) {
        ...
        this.tokens.footnotes[identifier] = {
          ...
          identifier,
          footnoteId: (0, _utils.getUniqueId)()
        };
        /* eslint-disable no-useless-escape */
        // Remove the footnote identifer prefix. eg: `[^identifier]: `.

        cap = cap[0].replace(/^\[\^[^\^\[\]\s]+?\]:\s+/gm, ''); // Remove the four whitespace before each block of footnote.
    ...
    if (cap) {
      item = {
        type: 'table',
        header: (0, _utils.splitCells)(cap[1].replace(/^ *| *\| *$/g, '')),
      ...
      if (item.header.length === item.align.length) {
        ...
        for (i = 0; i < item.cells.length; i++) {
          item.cells[i] = (0, _utils.splitCells)(item.cells[i], item.header.length);
        }
    ...
    if (cap) {
      ...
      let prevNext = true;
      let listItemIndices = [];
      let isTaskList = false; // Get each top-level item.
      ...
      for (; i < l; i++) {
        ...
        item = itemWithBullet;
        let newIsTaskListItem = false; // Remove the list item's bullet so it is seen as the next token.
        ...
        if (!newIsOrdered && this.options.gfm) {
          ...
          if (checked) {
            ...
            newIsTaskListItem = true; // Remove the list item's checkbox and adjust indentation by removing checkbox length.
