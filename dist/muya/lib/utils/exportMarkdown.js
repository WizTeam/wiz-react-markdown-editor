...
class ExportMarkdown {
  ...
  translateBlocks2Markdown(blocks, indent = '', listIndent = '') {
    const result = []; // helper for CommonMark 264
        ...
        case 'li':
          {
            ...
            this.isLooseParentList = insertNewLine; // if (insertNewLine) {
            //   this.insertLineBreak(result, indent);
            // }
        ...
        case 'ul':
          {
            ...
            } = block.children[0]; // if (lastListBullet && lastListBullet !== bulletMarkerOrDelimiter) {
            //   insertNewLine = false;
            // }
  ...
  normalizeTable(table, indent) {
    const result = []; // const { row, column } = table;
