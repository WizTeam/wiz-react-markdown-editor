...
const loadImage = async (url, detectContentType = false) => {
...
const getPageTitle = (url) => {
  ...
  req.send(); // Resolve empty string when `TIMEOUT` passed.
...
const getImageInfo = (src, baseUrl = window.DIRNAME) => {
  const imageExtension = _config.IMAGE_EXT_REG.test(src);
...
const mixins = (constructor, ...object) => {
...
const sanitize = (html, options) => {
  return (0, _dompurify.default)(escapeInBlockHtml(html), options);
};
...
exports.collectFootnotes = collectFootnotes;
