...
class ImageSelector extends _baseFloat.default {
  constructor(muya, options) {
    ...
    _defineProperty(this, "searchPhotos", keyword => {
      ...
      this.loading = true;
      this.photoList = [];
      this.unsplash.search.photos(keyword, 1, 40).then(_unsplashJs.toJson).then(json => {
        this.loading = false;
    ...
    _defineProperty(this, "replaceImageAsync", async ({
      alt,
      src,
      title
    }) => {
    ...
    _defineProperty(this, "renderBody", () => {
      ...
      if (tab === 'select') {
        bodyContent = [(0, _snabbdom.h)('button.muya-button.role-button.select', {
          on: {
            click: event => {
              this.handleSelectButtonClick();
            }
          }
        }, 'Choose an Image'), (0, _snabbdom.h)('span.description', 'Choose image from your computer.')];
      } else if (tab === 'link') {
        ...
        const inputWrapper = isFullMode ? (0, _snabbdom.h)('div.input-container', [altInput, srcInput, titleInput]) : (0, _snabbdom.h)('div.input-container', [srcInput]);
        const embedButton = (0, _snabbdom.h)('button.muya-button.role-button.link', {
          on: {
            click: event => {
        ...
        const bottomDes = (0, _snabbdom.h)('span.description', [(0, _snabbdom.h)('span', 'Paste web image or local image path. Use '), (0, _snabbdom.h)('a', {
          on: {
            click: event => {
              this.toggleMode();
            }
        ...
        if (this.loading) {
          ...
          const photos = this.photoList.map(photo => {
            const imageWrapper = (0, _snabbdom.h)('div.image-wrapper', {
              props: {
              ...
              on: {
                click: () => {
                  ...
                  this.unsplash.photos.getPhoto(id).then(_unsplashJs.toJson).then(json => {
            ...
            const desCom = (0, _snabbdom.h)('div.des', ['By ', (0, _snabbdom.h)('a', {
              props: {
    ...
    eventCenter.subscribe('muya-image-selector', ({
      ...
      if (reference) {
        ...
        if (this.unsplash) {
          ...
          this.loading = true;
          this.unsplash.photos.listPhotos(1, 40, 'latest').then(_unsplashJs.toJson).then(json => {
            this.loading = false;
  ...
  async handleKeyUp(event) {
    ...
    if (key === _config.EVENT_KEYS.ArrowUp || key === _config.EVENT_KEYS.ArrowDown || key === _config.EVENT_KEYS.Tab || key === _config.EVENT_KEYS.Enter) {
      return;
    }
  ...
  renderHeader() {
    ...
    const children = tabs.map(tab => {
      ...
      return (0, _snabbdom.h)(itemSelector, (0, _snabbdom.h)('span', {
        on: {
          click: event => {
