...
const parseSelector = (str = '') => {
  ...
  for (const tagName of _config.HTML_TAGS) {
    if (str.startsWith(tagName) && (!str[tagName.length] || /#|\./.test(str[tagName.length]))) {
      tag = tagName;
      if (_config.VOID_HTML_TAGS.indexOf(tagName) > -1) isVoid = true;
...
const tabCtrl = ContentState => {
  ...
  ContentState.prototype.indent = function () {
    ...
    const startBlock = this.getBlock(start.key);
    const endBlock = this.getBlock(end.key); // Handle `tab` to jump to the end of format when the cursor is at the end of format content.
