...
const clickCtrl = ContentState => {
  ContentState.prototype.clickHandler = function (event) {
    ...
    if (needRender) {
      ...
      requestAnimationFrame(() => {
        const cursor = _selection.default.getCursorRange();
  ...
  ContentState.prototype.setCheckBoxState = function (checkbox, checked) {
    ...
    checkbox.classList.toggle(_config.CLASS_OR_ID.AG_CHECKBOX_CHECKED);
  };

  ContentState.prototype.updateParentsCheckBoxState = function (checkbox) {
    let parent = (0, _getParentCheckBox.getParentCheckBox)(checkbox);

    while (parent !== null) {
      const checked = (0, _cumputeCheckBoxStatus.cumputeCheckboxStatus)(parent);

      if (parent.checked !== checked) {
        this.setCheckBoxState(parent, checked);
        parent = (0, _getParentCheckBox.getParentCheckBox)(parent);
      } else {
        break;
      }
    }
