...
const paragraphCtrl = ContentState => {
  ContentState.prototype.selectionChange = function (cursor) {
    ...
    } = cursor || _selection.default.getCursorRange();
    ...
    const cursorCoords = _selection.default.getCursorCoords();

    const startBlock = this.getBlock(start.key);
    const endBlock = this.getBlock(end.key);
    const startParents = this.getParents(startBlock);
    const endParents = this.getParents(endBlock);
    const affiliation = startParents.filter(p => endParents.includes(p)).filter(p => _config.PARAGRAPH_TYPES.includes(p.type));
  ...
  ContentState.prototype.handleListMenu = function (paraType, insertMode) {
    const {
      ...
      affiliation
    } = this.selectionChange(this.cursor);
    const {
      orderListDelimiter,
      bulletListMarker,
      preferLooseListItem
    ...
    if (isListed.length && !insertMode) {
      ...
      if (start.key === end.key || start.block.parent && start.block.parent === end.block.parent) {
        ...
        if (listType === 'task') {
          ...
          const listItemParagraph = this.updateList(paragraph, 'bullet', undefined, block); // 2. second update bullet list to task list
  ...
  ContentState.prototype.handleCodeBlockMenu = function () {
    ...
    if (affiliation.length && affiliation[0].type === 'pre' && /code/.test(affiliation[0].functionType)) {
      ...
      if (start.key === end.key) {
        ...
        const markdown = new _exportMarkdown.default(children.slice(startIndex, endIndex + 1), listIndentation).generate();
        const codeContent = this.createBlock('span', {
          text: markdown,
          lang,
          functionType: 'codeContent'
  ...
  ContentState.prototype.handleQuoteMenu = function (insertMode) {
    ...
    const isBlockQuote = affiliation.slice(0, 2).filter(b => /blockquote/.test(b.type)); // change blockquote to paragraph
  ...
  ContentState.prototype.insertContainerBlock = function (functionType, block) {
    ...
    const value = anchor.type === 'p' ? anchor.children.map(child => child.text).join('\n').trim() : '';
  ...
  ContentState.prototype.updateParagraph = function (paraType, insertMode = false) {
    ...
    let needDispatchChange = true; // Only allow valid transformations.
    ...
    switch (paraType) {
      ...
      case 'ol-order':
        {
          if (text && start.key === end.key) {
            this.cursor = {
              start: {
                key: start.key,
                offset: start.offset === end.offset ? start.offset + 1 : start.offset
              },
              end: {
                key: start.key,
                offset: end.offset + 1
      ...
      case 'paragraph':
        {
          ...
          const headingStyle = _config.DEFAULT_TURNDOWN_CONFIG.headingStyle;
          const parent = this.getParent(block); // \u00A0 is &nbsp;
          ...
          let newText = newLevel > 0 ? '#'.repeat(newLevel) + "".concat(String.fromCharCode(160)).concat(partText) // &nbsp; code: 160
          : partText; // Remove <hr> content when converting to paragraph.
  ...
  ContentState.prototype.insertParagraph = function (location, text = '', outMost = false) {
    ...
    } = this.cursor; // if cursor is not in one line or paragraph, can not insert paragraph
  ...
  ContentState.prototype.duplicate = function () {
    ...
    const cursorBlock = this.firstInDescendant(copiedBlock); // set cursor at the end of the first descendant of the duplicated block.
  ...
  ContentState.prototype.isSelectAll = function () {
    ...
    const lastTextBlock = this.getLastBlock(); // const { start, end } = this.cursor;
    ...
    return firstTextBlock.key === start.key && start.offset === 0 && lastTextBlock.key === end.key && end.offset === lastTextBlock.text.length && !this.muya.keyboard.isComposed;
  };

  ContentState.prototype.isSelectLine = function ({
    start,
    end
  }) {
    const startBlock = this.getBlock(start.key);
  ...
  ContentState.prototype.selectAll = function () {
    ...
    const startBlock = this.getBlock(start.key);
    const endBlock = this.getBlock(end.key); // handle selectAll in table.
    ...
    if (startBlock.functionType === 'cellContent' && endBlock.functionType === 'cellContent') {
      if (start.key === end.key) {
        ...
        const endTable = this.closest(endBlock, 'table'); // Check whether both blocks are in the same table.
  ...
  ContentState.prototype.getTypeFromBlock = function (block) {
    ...
    if (headingMatch && headingMatch[1]) {
      internalType = "heading ".concat(headingMatch[1]);
    }
      ...
      case 'span':
        {
          ...
          const listTypes = affiliation.slice(0, 3) // the third entry should be the ul/ol
          .filter(b => /ul|ol/.test(b.type)).map(b => b.listType); // Prefer list or blockquote over paragraph.
