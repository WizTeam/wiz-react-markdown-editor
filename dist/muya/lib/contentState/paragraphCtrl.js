...
const paragraphCtrl = (ContentState) => {
  ContentState.prototype.selectionChange = function (cursor) {
    const { start, end } = cursor || _selection.default.getCursorRange();
    ...
    const affiliation = startParents
      ...
      .filter((p) => _config.PARAGRAPH_TYPES.includes(p.type));
  ...
  ContentState.prototype.handleCodeBlockMenu = function () {
    ...
    if (
      ...
      if (start.key === end.key) {
        ...
        const markdown = new _exportMarkdown.default(
  ...
  ContentState.prototype.updateParagraph = function (paraType, insertMode = false) {
    ...
    switch (paraType) {
      ...
      case 'paragraph': {
        ...
        let newText =
          newLevel > 0
            ? '#'.repeat(newLevel) + ''.concat(String.fromCharCode(160)).concat(partText) // &nbsp; code: 160
  ...
  ContentState.prototype.insertParagraph = function (location, text = '', outMost = false) {
    const { start, end } = this.cursor; // if cursor is not in one line or paragraph, can not insert paragraph
  ...
  ContentState.prototype.isSelectLine = function ({ start, end }) {
    ...
    const { start: cursorStart, end: cursorEnd } = _selection.default.getCursorRange();
    ...
    switch (type) {
      case 'span': {
        ...
        const listTypes = affiliation
          ...
          .map((b) => b.listType); // Prefer list or blockquote over paragraph.
