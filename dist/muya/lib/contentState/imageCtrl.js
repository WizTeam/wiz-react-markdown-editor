...
const imageCtrl = ContentState => {
  ...
  ContentState.prototype.insertImage = function ({
    ...
    if (_config.URL_REG.test(src)) {
      imgUrl = encodeURI(src);
    } else if (_config.DATA_URL_REG.test(src)) {
      imgUrl = src;
    } else {
    ...
    if (imageFormat.length === 1 && imageFormat[0].range.start !== startOffset && imageFormat[0].range.end !== endOffset) {
      ...
      this.cursor = {
        start: {
          ...
          key,
          offset: start + 2 + imageAlt.length
        }
      };
    } else if (key !== end.key) {
  ...
  ContentState.prototype.deleteImage = function ({
    ...
    this.singleRender(block); // Hide image toolbar and image transformer
  ...
  ContentState.prototype.selectImage = function (imageInfo) {
    ...
    this.cursor = {
      start: {
        key,
        offset: imageInfo.token.range.end
      },
      end: {
        key,
        offset: imageInfo.token.range.end
