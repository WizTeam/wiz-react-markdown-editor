...
const updateCtrl = ContentState => {
  ...
  ContentState.prototype.checkNeedRender = function (cursor = this.cursor) {
    ...
    for (const token of (0, _parser.tokenizer)(startBlock.text, {
      labels,
    ...
    for (const token of (0, _parser.tokenizer)(endBlock.text, {
      labels,
      options: this.muya.options
    })) {
      if (NO_NEED_TOKEN_REG.test(token.type)) continue;
  ...
  ContentState.prototype.updateBlockQuote = function (block, line) {
    ...
    this.cursor = {
      start: {
        key,
        offset: Math.max(0, start.offset - 1)
      },
      end: {
        key,
        offset: Math.max(0, end.offset - 1)
      }
    };
    return quoteBlock;
  };
