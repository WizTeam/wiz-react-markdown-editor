...
const backspaceCtrl = ContentState => {
  ContentState.prototype.checkBackspaceCase = function () {
    ...
    const outMostParagraph = (0, _dom.findOutMostParagraph)(node);
  ...
  ContentState.prototype.backspaceHandler = function (event) {
    ...
    if (parentNode && parentNode.classList.contains('ag-inline-image')) {
      if (_selection.default.getCaretOffsets(node).left === 0) {
        event.preventDefault();
        event.stopPropagation();
        const imageInfo = (0, _getImageInfo.getImageInfo)(parentNode);
        return this.deleteImage(imageInfo);
      }

      if (_selection.default.getCaretOffsets(node).left === 1 && right === 0) {
        event.stopPropagation();
        event.preventDefault();
    ...
    if (block.type === 'span' && block.functionType === 'paragraphContent' && left === 0 && preBlock && preBlock.functionType === 'footnoteInput') {
      ...
      if (!block.nextSibling) {
        ...
        const lineBlock = this.createBlock('span', {
          text: block.text
        });
